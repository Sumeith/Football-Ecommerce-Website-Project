...
if (isset($_GET['deleteid'])) {
	echo 'Do you really want to delete product with ID of ' . $_GET['deleteid'] . '? <a href="inventoryList.php?yesdelete=' . $_GET['deleteid'] . '">Yes</a> | <a href="inventoryList.php">No</a>';
	exit();
}
if (isset($_GET['yesdelete'])) {
	...
	$sql = mysql_query("DELETE FROM catalogue WHERE id='$id_to_delete' LIMIT 1") or die (mysql_error());
...
//$row_count = mysql_query("SELECT COUNT (*) FROM catalogue");
...
if (isset($_POST['name'])) {
	...
	$productMatch =mysql_query("SELECT id COUNT(*) FROM products WHERE name='$product_name' LIMIT 1"); // count the output amount
    if ($productMatch > 0) {
		echo 'Sorry you tried to place a duplicate "Product Name" into the system, <a href="inventoryList.php">click here</a>';
		exit();
	}
	// Add this product into the database
	$sql = mysql_query("INSERT INTO catalogue (id, category, name, description, brand, qty, price) 
        VALUES('$row_count','$category','$product_name','$description','$brand','$qty','$price')") or die (mysql_error());
...
if (sizeof($res_array) > 0) {
    ...
    foreach($res_array as $namer){
        $product_list = $product_list . $namer["id"].".". "\t". $namer["name"]."\t"."______PRICE:R". $namer["price"]."\t"."______QTY:"."\t".$namer["qty"] . "<br>"."<br>";
    }
    unset($namer); 
...
<div align="center" id="mainWrapper">
  ...
  <div id="pageContent"><br />
    <div align="right" style="margin-right:32px;"><a href="inventoryList.php#inventoryForm">+ Add New Inventory Item</a></div>
